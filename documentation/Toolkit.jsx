...
const BASE_TOOLTIP_KEYS=[
  ...
  "slime","slimeballs","soul_berry","spider_eye","squid_ink","strider_meatballs","sugar",
...
const ITEM_TYPE_OPTIONS=[
  ...
  {v:"bowlFood",          l:"bowlFood",             hasFood:true,  noDisplay:true,  desc:"Food item that returns a bowl on use. Stack 16. Shows effects in tooltip. Use for soups and stews."},
...
function genBlockCuttingRecipe(id,sliceId,sliceCount,ns="createfood"){return{type:"farmersdelight:cutting",ingredients:[{tag:"c:"+id}],tool:{tag:"c:tools/knife"},result:[{item:{count:sliceCount,id:ns+":"+sliceId}}],"neoforge:conditions":[{type:"createfood:enabled",id:sliceId}]};}
function genSlicesCombineRecipe(id,sliceId,sliceCount,ns="createfood"){return{type:"minecraft:crafting_shapeless",ingredients:Array.from({length:sliceCount},()=>({tag:"c:"+sliceId})),result:{id:ns+":"+id},"neoforge:conditions":[{type:"createfood:enabled",id}]};}
...
function buildItemJava(item){
  const{id,itemType,nutrition,saturation,fast,craftRemainder,effects,tooltipKeys,isCompat,compatKey}=item;
  if(!id)return"";
  ...
  if(itype==="bowlConsumable"||itype==="bowlFood") return"public static final DeferredItem<Item> "+idU+" = bowlFood(\""+id+"\", "+nut+", "+sat+t+");";
...
function buildBowlJava(item){const{id,bowlHasFood,bowlNutrition,bowlSaturation,bowlFast,bowlEffects,bowlTooltipKeys,tooltipKeys,isCompat,compatKey}=item;return buildItemJava({id:id+"_bowl",itemType:"bowlFood",hasFood:bowlHasFood!==false,nutrition:bowlNutrition||4,saturation:bowlSaturation||0.6,fast:bowlFast||false,effects:bowlEffects||[],tooltipKeys:bowlTooltipKeys||tooltipKeys||[],isCompat:isCompat||false,compatKey:compatKey||""});}
...
function cond(id){return{"neoforge:conditions":[{type:"createfood:enabled",id}]};}
...
function buildRecipeJson(rec){
  ...
  const parsed=parseInputs(inputs),c=cond(baseId);
...
const genBottleFilling=id=>({type:"create:filling",ingredients:[{item:"minecraft:glass_bottle"},{type:"fluid_tag",amount:250,fluid_tag:"c:"+id}],results:[{item:{id:"createfood:"+id+"_bottle"}}],"neoforge:conditions":[{type:"createfood:enabled",id:id+"_bottle"}]});
const genBottleEmptying=id=>({type:"create:emptying",ingredients:[{tag:"c:"+id+"_bottle"}],results:[{item:{id:"minecraft:glass_bottle"}},{amount:250,id:"createfood:"+id}],"neoforge:conditions":[{type:"createfood:enabled",id}]});
const genBowlFilling=id=>({type:"create:filling",ingredients:[{item:"minecraft:bowl"},{type:"fluid_tag",amount:333,fluid_tag:"c:"+id}],results:[{item:{id:"createfood:"+id+"_bowl"}}],"neoforge:conditions":[{type:"createfood:enabled",id:id+"_bowl"}]});
const genBowlEmptying=id=>({type:"create:emptying",ingredients:[{tag:"c:"+id+"_bowl"}],results:[{item:{id:"minecraft:bowl"}},{amount:333,id:"createfood:"+id}],"neoforge:conditions":[{type:"createfood:enabled",id}]});
const genBottleFromBucket=id=>({type:"minecraft:crafting_shapeless",ingredients:[{tag:"c:"+id+"_bucket"},...Array(4).fill({item:"minecraft:glass_bottle"})],result:{id:"createfood:"+id+"_bottle",count:4},"neoforge:conditions":[{type:"createfood:enabled",id:id+"_bottle"}]});
const genBucketFromBottles=id=>({type:"minecraft:crafting_shapeless",ingredients:[{item:"minecraft:bucket"},...Array(4).fill({tag:"c:"+id+"_bottle"})],result:{id:"createfood:"+id+"_bucket"},"neoforge:conditions":[{type:"createfood:enabled",id:id+"_bottle"}]});
const genBowlFromBucket=id=>({type:"minecraft:crafting_shapeless",ingredients:[{tag:"c:"+id+"_bucket"},...Array(3).fill({item:"minecraft:bowl"})],result:{id:"createfood:"+id+"_bowl",count:3},"neoforge:conditions":[{type:"createfood:enabled",id:id+"_bowl"}]});
const genBucketFromBowls=id=>({type:"minecraft:crafting_shapeless",ingredients:[{item:"minecraft:bucket"},...Array(3).fill({tag:"c:"+id+"_bowl"})],result:{id:"createfood:"+id+"_bucket"},"neoforge:conditions":[{type:"createfood:enabled",id:id+"_bowl"}]});
...
const BLANK_REC=()=>({uid:Math.random().toString(36).slice(2),recipeType:"create:deploying",outputId:"",outputCount:1,isFluidOutput:false,fluidAmount:250,inputs:[{type:"tag",value:""}],experience:0.0,cookingtime:200,altSuffix:"",shapedPattern:["XXX","XXX","XXX"],shapedKey:[{letter:"X",inputType:"tag",input:""}]});
function parseImportedJson(fn,txt){try{const o=JSON.parse(txt);if(o.type&&(o.results||o.result||o.ingredient)){const ia=Array.isArray(o.ingredients)?o.ingredients:(o.ingredient?[o.ingredient]:[]);const inputs=ia.map(i=>{if(i.fluid_tag)return{type:"fluid_tag",value:i.fluid_tag+"|"+(i.amount||250)};if(i.tag)return{type:"tag",value:i.tag};if(i.item)return{type:"item",value:i.item};return{type:"tag",value:""};});const oid=((o.results&&o.results[0]?(o.results[0].item?o.results[0].item.id:o.results[0].id):(o.result?o.result.id:""))||"").replace("createfood:","");const rtype=o.type==="create:mixing"&&o.heat_requirement==="heated"?"create:mixing_heated":o.type==="create:compacting"&&o.heat_requirement==="heated"?"create:compacting_heated":o.type;const cnt=((o.results&&o.results[0]?o.results[0].count:(o.result?o.result.count:1))||1);return{type:"recipe",rec:{...BLANK_REC(),recipeType:rtype,outputId:oid,outputCount:cnt,isFluidOutput:!!(o.results&&o.results[0]&&o.results[0].amount),fluidAmount:(o.results&&o.results[0]&&o.results[0].amount)||250,inputs,experience:o.experience||0.0,cookingtime:o.cookingtime||200}};}if(o.values!==undefined)return{type:"tag",ids:(o.values||[]).map(v=>(v.id||v).replace("createfood:",""))};if(Object.keys(o).some(k=>k.startsWith("item.createfood."))){const items={};for(const[k,v]of Object.entries(o))if(k.startsWith("item.createfood."))items[k.replace("item.createfood.","")]=v;return{type:"lang",items};}}catch(e){}return null;}
      ...
      if(Array.isArray(o.items)&&o.items.length>0&&o.items[0]?.id){
        ...
        for(const spec of o.items){
          ...
          for(const r of(spec.recipes||[])){
            const rec={...BLANK_REC(),recipeType:r.type,outputId:r.output||spec.id,outputCount:r.count??1,inputs:(r.inputs||[]).map(inp=>({type:inp.type||"tag",value:inp.value||""})),altSuffix:r.suffix||"",isFluidOutput:r.fluidOutput||false,fluidAmount:r.fluidAmount||250,experience:r.experience||0,cookingtime:r.cookingtime||200};
...
function ItemPanel({item,onChange,sections,setSections,onSave,onReset,hasSaved,allItemFiles}){
  ...
  const addVarEffect=vi=>set("variants",(variants||[]).map((v2,j)=>j===vi?{...v2,effects:[...(v2.effects||[]),{effectId:EFFECTS[0].id,duration:1200,amplifier:0}]}:v2));
  ...
  const addVarBowlEffect=vi=>set("variants",(variants||[]).map((v2,j)=>j===vi?{...v2,bowlEffects:[...(v2.bowlEffects||[]),{effectId:EFFECTS[0].id,duration:1200,amplifier:0}]}:v2));
  ...
  return (<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,height:"100%",minHeight:0}}>
    <div style={{overflowY:"auto",paddingRight:4}}>
      ...
      <Section title="Registration" open={openMap.registration} onToggle={()=>toggle("registration")}>
        ...
        {isItem&&isCF&&<><Sel label="Item Type" value={itemType||"food"} onChange={v=>{set("itemType",v);openNext("registration");}} options={ITEM_TYPE_OPTIONS.map(o=>({v:o.v,l:o.l}))} note={itemTypeInfo(itemType||"food").desc}/>{itemType==="plainCr"&&<Sel label="Remainder" value={craftRemainder} onChange={v=>set("craftRemainder",v)} options={CRAFT_REMAIN}/>}</>}
...
function ImportExportPanel({itemFiles,recipeFiles,autoRecipeFiles,sections,setSections,item,recipeState,advisorState,setAdvisorState,savedItemSnapshots,savedItemFiles,savedRecipeFiles2,onBatchImport}){
  ...
  const addItemChecklist=(snap)=>{
    ...
    if(sIsItem&&sCF){
      ...
      if(snap.isCompat)clFiles.push("ModConfig.java — add "+sId+" to disable_items");
