...
public class LargeBowlFluidRenderer implements BlockEntityRenderer<LargeBowlBlockEntity> {
    ...
    public LargeBowlFluidRenderer(BlockEntityRendererProvider.Context context) {
    }

    @Override
    public void render(LargeBowlBlockEntity bowl, float partialTick, PoseStack poseStack,
        ...
        float fill = Math.min(1.0f, (float) bowl.getAmount() / Services.PLATFORM.getLargeBowlCapacityMb());
    ...
    private static TextureAtlasSprite resolveSprite(Fluid fluid) {
        ...
        String texture = id.getNamespace().equals(CreateFoodCommon.MOD_ID)
