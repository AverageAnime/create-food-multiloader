...
public class BottleFoodBlock extends DisplayFoodBlock {
    ...
    public BottleFoodBlock(Supplier<Item> displayItem, double heightInPixels, boolean hasParticles,
        ...
        super(BlockBehaviour.Properties.copy(Blocks.GLASS), displayItem, 1);
    ...
    protected void applyEatEffects(ItemStack copy, FoodProperties props, Player player,
        ...
        for (var entry : props.getEffects()) {
            if (level.random.nextFloat() < entry.getSecond()) {
                player.addEffect(new MobEffectInstance(entry.getFirst()));
            }
        }
    }
    ...
    @Override
    public SoundEvent getAddSound() { return SoundEvents.BOTTLE_FILL; }

    @Override
    protected SoundEvent getRemoveSound() { return SoundEvents.BOTTLE_EMPTY; }
